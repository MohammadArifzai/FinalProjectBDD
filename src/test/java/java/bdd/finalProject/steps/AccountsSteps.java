package java.bdd.finalProject.steps;


import java.bdd.finalProject.pages.AccountPage;
import java.bdd.finalProject.utility.SeleniumUtilities;
...
import static java.bdd.finalProject.base.BaseSetup.getDriver;
...
public class AccountsSteps extends SeleniumUtilities {
    ...
    public void user_should_see_five_rows_per_page_by_default() {
        Object AccountsPage;
        List<WebElement> rows = getDriver().findElements(AccountPage.PRIMARY_ACCOUNTS_ALL_ROWS);
    ...
    public void user_selects_the_show_per_page_option_as(String selectedNumber) {
        selectFromDropDownByValue(AccountPage.SHOW_PER_PAGE_OPTION, selectedNumber);
    }

    @Then("user should see (.*) rows per page$")
    public void user_should_see_rows_per_page(String expectedRows) {
        int actualRows = getDriver().findElements(AccountPage.PRIMARY_ACCOUNTS_ALL_ROWS).size();
    ...
    public void userClicksOnTheAccountsButton() {
    }
    ...
    public void userShouldBeNavigatedToCustomerServicePortal() {
    }
}
